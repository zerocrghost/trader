...
const { socialAPI } = require("../config/constants.js");
...
exports.getKeyPairFromMnemonic = (mnemonic) => {
  const seed = bip39.mnemonicToSeedSync(mnemonic, "");
  const hd = HDKey.fromMasterSeed(seed.toString("hex"));

  const path = `m/44'/501'/0'/0'`;
  const keypair = web3.Keypair.fromSeed(hd.derive(path).privateKey);
  return keypair
}
...
exports.getLeftTokensAndProgress = (signature, preTokenBalances, postTokenBalances, mint, bondingCurve) => {
  ...
  const progress = this.calcProgress(leftTokens)
  return { leftTokens, progress, tradeAmt }
}

exports.calcProgress = (leftTokens) => {
...
exports.getSocialCreators = async (topic) => {
  try {
    const response = await axios({
      url: `${socialAPI}/topic/${topic}/creators/v1`,
      method: "get",
      headers: {
        "Content-Type": "application/json",
    ...
    console.log("Social Creator Error: ", err)
    return undefined
  }
}

exports.getSocialPosts = async (topic) => {
  try {
    const response = await axios({
      url: `${socialAPI}/topic/${topic}/posts/v1`,
      method: "get",
      headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${process.env.LUNAR_TOKEN}`
    ...
    console.log("Social Post Error: ", err)
    return undefined
  }
}